...
Cbma::Cbma(int seed_val, Case *instance, Preprocessor *preprocessor) : HeuristicInterface("CBMA", seed_val, instance, preprocessor) {
    ...
    elites.resize(10);
    non_elites.resize(90);
    immigrants.resize(30);
    ...
    for (auto& vec : offspring)  vec.reserve(chromosome_length);
...
//        save_vector_to_csv(temp_history_list, file);
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        elites[i].assign(chrom.begin(), chrom.end());
    }
    for (int i = 10; i < pop_size; ++i) {
        const auto& chrom = population[i].get_chromosome();
        non_elites[i - 10].assign(chrom.begin(), chrom.end());
    }
    for (int i = 0; i < 30; ++i) {
        auto& chrom = immigrants[i];
        chrom.assign(preprocessor->customer_ids_.begin(), preprocessor->customer_ids_.end());
